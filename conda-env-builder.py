...
def conda_build(environment_filename, install_directory, state_directory, permissions=None, uid=None, gid=None):
    environment_name = yaml.safe_load(environment_filename.open())['name']

    environment_hash = filename_hash(environment_filename)
    environment_directory = state_directory / f'{environment_hash}-{environment_name}'
    environment_install_directory = install_directory / environment_name

    build_key = f'{environment_hash}-{environment_name}'
    log_filename = state_directory / '.logs' / f'{datetime.datetime.now()}-{build_key}.log'
...
def _conda_build(environment_name, environment_filename, environment_directory, environment_install_directory, permissions, uid, gid):
    ...
    # "state_directory/environment_hash" installation is guarenteed to
...
def main():
    ...
    state_directory = pathlib.Path(args.state).expanduser().resolve()
    ...
    while True:
        ...
        for filename in environment_filenames:
            conda_build(filename, output_directory, state_directory, args.permissions, args.uid, args.gid)
...
def init_cli():
    ...
    parser.add_argument('-s', '--state', type=str, default='.conda-env-builder-cache', help='state directory for storing environments')
    parser.add_argument('-o', '--output', type=str, help='output directory for conda environment builds', required=True)
